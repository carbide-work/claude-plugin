...
Version `1.0.0` removes the PaintScout inbound-request configuration tool. PaintScout quote outbound sync now creates a draft only after an appointment is booked and uses TypeSafe AI to select the best active quote type.
...
git push git@github.com:carbide-work/claude-plugin.git claude-plugin-release:refs/tags/v1.0.0
...
6. Confirm MCP initialization and both portable plugin manifests advertise version `1.0.0`.
